...
### 乘积分解定理（Product Decomposition）
&ensp;&ensp;&ensp;&ensp;10个0-1变量所组成的条件概率表的行数就已经达到了$2^{10}$之多，那么对于实际中更多的变量及其取值种类，计算机将无法容纳如此巨大的条件概率表。而图模型可以极大地减少条件概率的存储难题。在概率论中，n个变量的条件概率有如下的乘法计算公式：$$P(X_1,X_2,X_3,X_4,...,X_n)=P(X_1)P(X_2|X_1)P(X_3|X_1,X_2)P(X_4|X_1,X_2,X_3)...P(X_n|X_1,X_2,...,X_{n-1})$$
...
## 从数据中得到贝叶斯网络
### 基于计分的算法（score-based algorithms）
...
### 基于约束的算法（constraint-based algorithms）
...
data <- read.csv("driver.csv",sep=",",header=TRUE)
...
data <- read.csv("cgss2017.csv", header=TRUE, sep=",",na.strings = c(9999999,9999998,9999997,9999996,99,98,97,14,21,"#NULL!"))
