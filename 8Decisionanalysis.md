...
其中$ M_i $和$ S_i $由模型估计的后验模拟计算得到。均值是$ M_i = X_i \hat{\beta} + \hat{\alpha_{j(i)}} $，其中$ X_i $是一个列向量，包含住宅i的房屋水平预测指标(指示房屋是否有地下室以及地下室是否为生活区的指标)，($ \hat{\beta} $，$ \hat{\alpha} $)是该国相应地区分析的后均值。方差$ S_i ^2 $(由早期后验计算得到)包括系数$ \alpha $,$ \beta $中的后验不确定性，以及层次方差分量$ \tau^2 $和$ \kappa^2 $。(我们预测的是实际的氡水平，而不是测量值，因此$ \sigma^2 $在这里不起作用。)结果表明，家庭氡水平预测分布的几何标准差$ e^S $在2.1到3.0之间变化，大多数美国家庭的几何标准差$ e^S $在(2.1、2.5)范围内。($ e^S > 2.5 $的房屋位于人口较少的县，在氡调查中获得的信息很少，因此这些县的预测不确定性相对较高。)房屋预测分布的几何平均值$ e^M $在0.1到14.6pCi/L之间变化，95%在0.3、3.7范围内，50%在0.6、1.6范围内。预测几何均值最高的住宅为高氡水平的县的地下室住宅;预测几何平均值最低的房子没有地下室，而且位于氡含量低的县。
