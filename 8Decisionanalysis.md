...
如果我们愿意对所有县进行简化假设，即$ \sigma = \log(1.2) $和$ S = \log(2.3) $，那么我们可以通过给出阈值水平$ M_{low} $和$ M_{high} $来总结决策建议，如果$ M > M_{high} $，则首选决策1（立即补救），如果$ M < M_{low} $，则首选决策2（不监测或补救），如果$ M \in [M_{low},M_{high}] $。图9.5显示了这些截止值作为$ R_{action} $的函数，因此显示了推荐决策作为($ R_{action} $，e^M)的函数。例如，将$ R_{action}=4pCi/L $设置为以下基于e^M的建议，即基于您所在的县和房屋类型的家庭氡的先前GM：
