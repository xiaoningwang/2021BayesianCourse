...
```
def calc_Hd(dataset):
    all = dataset[y_lable].count() #all=统计总行数
    calcEnt = df(dataset.groupby(y_lable)[y_lable].count()) #统计y分别取值的数量
    calcEnt["Ent"]=calcEnt[y_lable].apply(lambda x:-x/all*log(x/all,2)) #计算y分别取值的HD
    HD = round(calcEnt["Ent"].sum(),3)  #计算总体的HD
    print("总体经验熵："+str(HD))
    return HD
```
...
构建决策树代码：
```
def createTree(x_lables,total):
    classList = list(total[y_lable])    #取分类标签
    if len(x_lables) == 0:              #若拆分后已经没有X了，则返回y的众数，对应第一个条件
        return majorityCnt(classList)
    if len(set(classList)) == 1:        #若Y只有一类，则返回，对应第二个条件
        return classList[0]
    Ent = {}                            #存储各个X信息增益的字典
    HD = calc_Hd(total)                 #计算总体经验熵
    best_Ent = 0
    best_lable = ""
    for lable in x_lables:              #循环每个X分别计算条件经验熵，并将求得信息增益存储值Ent字典中
        Hyd =calc_Hyd(total, lable)
        Ent[lable] = round(HD - Hyd, 3) #循环判断哪个X的信息增益最大
        if Ent[lable] >= best_Ent:
            best_Ent = Ent[lable]
            best_lable = lable
        x_lables.remove(best_lable)     #删除已经使用的特征标签
        mytree = {best_lable:{}}        #用字段构建决策树
        featValues = set(total[best_lable]) #得到数据集中所有最优特征的属性值
        for value in featValues:
            mytree[best_lable][value] = createTree(x_lables,splitDataSet(total,best_lable,value))
        return mytree
```
