...
决策树由三个分支组成。在每个分支，我们以美元为单位评估预期损失，使用$ D_r = 2000/(R_{action}−R_{remed}) $作为额外家庭氡暴露的每pCi/L等效成本，将氡暴露(超过30年)换算成美元价值。在下面的表达式中，我们设$ R = e^\theta $为所考虑的家中未知的氡暴露水平；先验分布和后验分布为$ θ = \logR $的正态分布。
...
&emsp; (一)进行补救措施。预期损失是由于测量的直接损失加上决策1计算的补救损失，但使用的是后验分布而不是先验分布:$$ L_{3a} = 50 + D_r\frac{1}{30}e^{M+\frac{1}{30}S^2} + 2000 + D_r[R_{remed}\Phi(\frac{\Gamma-\log(R_{remed})}{\sqrt{V}}) + e^{\Gamma+\frac{1}{2}V}(1 - \Phi(\frac{\Gamma + V - \log(R_{remed})}{\sqrt{V}}))] $$

其中$ \Gamma $和V是$ \theta $分布中$ \Gamma $和V的后验均值和方差。

&emps; (二)不补救。预期损失:$$ L_{3b} = 50 + D_r\frac{1}{30}e^{M+\frac{1}{2}S^2} + D_re^{\Gamma + \frac{1}{2}V} $$

决定是否纠正给定的度量。为了评估决策树，我们必须首先考虑3(a)和3(b)之间的内在决定，条件是在测量y上。设$ y_0 $为点（在对数刻度上），在该点上，如果$ y > y_0 $，则选择进行修正；如果$ y < y_0 $，则选择不进行修正。（由于测量误差，$ y \neq \theta $，因此$ y_0 \neq \log(R_{action}) $。）我们通过数值求解隐式方程来确定$ y_0 $，它取决于先验平均值M、先验标准偏差S和测量标准偏差$ \sigma $,方程如下：$$ L_{3a} = L_{3b} at y = y_0 $$
...
如果我们愿意对所有县进行简化假设，即$ \sigma = \log(1.2) $和$ S = \log(2.3) $，那么我们可以通过给出阈值水平$ M_low $和$ M_high $来总结决策建议，如果$ M > M_high $，则首选决策1（立即补救），如果$ M < M_low $，则首选决策2（不监测或补救），如果$ M \in [M_low,M_high] $。图9.5显示了这些截止值作为$ R_action $的函数，因此显示了推荐决策作为($ R_action $，e^M)的函数。例如，将R_action=4pCi/L设置为以下基于e^M的建议，即基于您所在的县和房屋类型的家庭氡的先前GM：
...
图2地图显示:(a)在$ R_action = 4pCi/L $的完美信息行动水平下，建议测量的每个县的房屋比例;(b)一旦进行了测量，每个县建议进行补救的房屋的预期比例。对于目前的氡模型，在任何一个县，是否测量和是否补救的建议只取决于房屋类型:房屋是否有地下室和地下室是否用作生活空间。犹他州和南卡罗莱纳边界的明显不连续是由这些州进行的氡测量的不规范造成的，我们在这里忽略这个问题。
...
图2显示了基于4pCi/L行动级别$ R_action $的推荐测量（以及一年后推荐的补救措施）的地理格局。每个县都根据建议进行测量（然后进行修复）的房屋比例进行遮荫。这些建议在预测县内氡分布的模型中纳入了参数不确定性的影响，因此，随着更好的预测出现，这些地图预计将有所改变。
...
2.  对所有房屋进行长期测量，然后对测量超过规定氡作用水平{R_action}的房屋进行补救。
3.  对所有房屋进行短期测量，然后对偏差校正测量超过指定氡作用水平{R_action}的房屋进行补救（偏差由层次回归模型估计）。
4.  对所有房屋进行短期测量，对未校正的测量超过规定的氡作用水平的房屋进行补救。

我们根据节省的总生命和美元成本来评估上述每一种策略，这些结果由氡行动水平{R_action}参数化。挽救的生命和成本都被考虑为30年。对于每一种策略，我们假设所有房屋的等级反应都是相同的（这相当于一个统一的国家建议）。为了计算挽救的生命，我们假设每栋房子的家庭构成与美国的平均水平相同。我们通过汇总全国各个家庭的决策，来评估预期成本和预期挽救的生命数量。在我们的模型中，我们只需要考虑3078个县中每个县定义的三种房屋类型（没有地下室、地下室不作为居住空间、地下室作为居住空间）。

我们只详细描述基于层次模型的决策策略的预期成本和预期寿命计算的结果。如果所有地方都采用$ R_action = 4pCi/L $(如图2地图所示)的策略，美国7000万地面接触房屋中约26%将进行监测，4.5%将进行补救。被修复的房屋包括280万氡水平在4pCi/L以上的家庭(占所有此类家庭的74%)，84万氡水平在8pCi/L以上的家庭(占所有此类家庭的91%)。估计总费用为73亿美元——测量费用为10亿美元，矫正费用为63亿美元——预计将在30年内挽救4.9万名吸烟者和3.5万名非吸烟者的生命。其他行动层面和其他决策策略所节省的总成本和总生命也以同样的方式计算。

图3显示了四种策略在30年期间预期成本和预期挽救生命之间的权衡。曲线上的数字是行动级别，Raction。这个数字使我们能够比较同等预期成本或同等预期挽救生命的替代策略的有效性。例如，$ R_action = 4pCi/L $的推荐策略（图中实线）预计将挽救83000条生命，预计成本为73亿美元。让我们将其与EPA基于未经修正的短期测量（图中的虚线）含蓄推荐的策略进行比较。同样花费73亿美元，未经修正的短期策略预计只能挽救6.4万人的生命;为了达到同样的预期挽救83000条生命，未经纠正的短期策略将花费约120亿美元。

图3 &emsp; 各种氡测量/补救措施的预期挽救生命与预期成本。数字表示$ R_action $的值。实线是建议的只测量某些家庭的策略;另一种则假设所有的房屋都是测量过的。所有的结果都是美国30年的估计数据。
