...
对于样本集合D来说，随机变量X是样本的类别，假设样本有n个类别，每个类别的概率是$ \frac{|C_n|}{D} $，其中$ |C_n| $表示类别n的样本个数，|D|表示样本总数
计算信息增益（ID3算法）首先需要计算总体的经验熵和条件经验熵，总体的经验熵计算公式如下：$$ H(D) = -\sum_{n=1}^N{\frac{|C_n|}{|D|}\log_2\frac{|C_n|}{|D|}} $$
条件经验熵计算公式如下：$$ H(D|X) = \sum_{i=1}^n{p_iH(D|X=x_i)} $$
则X的信息增益计算Gain(X)公式为：$$ Gain(D，X) = H(D) - H(D|X) $$
若要继续计算信息增益比（C4.5算法），则由：$$ 信息增益比 = 信息增益 * 惩罚参数 $$
惩罚参数计算公式：$$ 惩罚参数 = \frac{1}{H_A (D)} = \frac{1}{-\sum_{i=1}^n{\frac{|D_i|}{|D|}\log_2\frac{|D_i|}{|D|}}} $$
最终得到X的信息增益比：$$ g_R (D,X) = \frac{Gain(D,X)}{H_A (D)} $$
