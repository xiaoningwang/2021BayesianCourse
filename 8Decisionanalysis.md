...
采取行动的适当氡水平$ R_action $取决于减少氡的美元价值和补救措施的效益。我们假设，补救措施使房屋居住区域的年平均氡水平在高于该水平时降至$ R_remed = 2pCi/L $水平，但在低于该水平时保持不变。然后，行动水平被确定为补救的收益($$ D_r (R_{action} − R_{remed}) $$)等于成本($2000)的值，$$ R_{action} = $2000/D_r + R_{remed} $$
...
我们同时校准数据并预测氡水平，通过在房屋和县这一级上使用预测器，将分层模型拟合到两组测量值中，并使用单独的模型拟合到美国10个地区中的每一个。让$ y_i $表示对县$ j_(i) $房子i的氡测量值的对数，X表示家庭级预测值矩阵，包括房子是否有地下室以及地下室是否为居住区的指标，如果测量值i为短期筛选测量值，则指标变量等于1。包括指示器，用于校正筛选测量中的偏差。我们假设一个正态线性回归模型，$$ y_i ~ N(X_i\beta+\alpha_{j(i)}，\sigma_i ^2), &emsp; for &emsp; houses &emsp; i = 1,...,n, $$

其中$ \alpha_j(i) $是县域效应，根据测量i是长期还是短期，数据级方差参数$ \sigma_i $可以有两个可能的值。县参数$ \alpha_j $也假定为正态分布，$$ \alpha_j ~ N(W_j\gamma + \delta_k(j),\tau^2), &emsp; for &emsp; counties &emsp; j = 1,...,J, $$

利用包括气候数据和土壤中铀含量测量在内的县级预测指标W，表征j县地质特征的指标$ \delta_k(j) $为K = 19类型之一。最后，根据数据估算了19种地质类型的$ δ_k $系数，$$ \delta_k ~ N(0,\kappa^2), &emsp; for &emsp; geologic &emsp; type k = 1,...,K, $$
