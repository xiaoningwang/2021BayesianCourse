...
对于任何应用问题建立模型通常有许多选择。我们通常的方法是从一个简单的模型开始，它只使用一些可用的信息------例如，在回归中没有使用一些可能的预测因素，将一个正常模型拟合到离散数据中，或忽略不等方差的证据，并拟合一个简单的等方差模型。一旦我们成功地拟合了一个简单的模型，我们就可以检查它对数据的拟合（如第6.3和6.4节中讨论的)，然后对其进行扩展(如第7.5节中讨论的）。
...
> **样例：对这八所学校的模型的预期预测精度**
>
> 在第5.5节的示例中，提出了三种推理模式：
>
> ·不混合：对8所学校的单独估计，反映了实验是独立进行的，所以每个学校的观察值是对其自身治疗效果的无偏估计。这个模型有八个参数：对每个学校的估计。
>
> ·完全混合：将所有学校的数据平均成一个数字，反映了这八所学校实际上非常相似（八种不同的处理方法)，也反映了八个估计之间的差异(表5.2中数字的左列)并没有比考虑到标准误差(表中最右的一列）单独预期的要大。此模型只有一个、共享的参数。
>
...
*表7.1偏差（-2倍对数预测密度）和使用AIC、DIC、WAIC拟合修正（使用校正后的$p_{WAIC2}$），并对表5.2中数据拟合的三个模型进行留一法交叉验证。AIC/DIC/WAIC值越低，意味着预测精度就越高。*
...
> 表7.1说明了使用预测对数密度和信息标准来比较三种模型------不混合、完全混合和与SAT教练数据的分层拟合。我们只有小组层面的数据，所以我们必须基于8所学校来定义我们的数据点和交叉验证，而不是个别学生。
>
> 我们将沿着表7.1的几行来了解这三个模型的不同信息标准是如何工作的，然后我们将讨论如何使用这些度量来比较模型。
>
